...
object KISA_SEED_CTR {
    ...
    private const val BLOCK_SIZE_CTR = 16
    private const val BLOCK_SIZE_CTR_INT = 4
    ...
    private fun SeedRound(T: IntArray, LR: IntArray, L0: Int, L1: Int, R0: Int, R1: Int, K: IntArray, K_offset: Int) {
        ...
        T[1] = T[1] xor T[0]
        T[1] = SS0[GetB0(T[1]).toInt() and 0x0ff] xor SS1[GetB1(
            T[1]
        ).toInt() and 0x0ff] xor SS2[GetB2(T[1]).toInt() and 0x0ff] xor SS3[GetB3(
            T[1]
        ).toInt() and 0x0ff]
        T[0] += T[1]
        T[0] = SS0[GetB0(T[0]).toInt() and 0x0ff] xor SS1[GetB1(
            T[0]
        ).toInt() and 0x0ff] xor SS2[GetB2(T[0]).toInt() and 0x0ff] xor SS3[GetB3(
            T[0]
        ).toInt() and 0x0ff]
    ...
    private fun UpdateCounter_for_SEED(pbOUT: IntArray, pbOUT_offset: Int, nIncreaseValue: Int, nMin: Int) {
        var bszBackup = 0

        if (0 > nMin) return

        if (0 < nMin) {
            val b = Common.get_byte_for_int(pbOUT, pbOUT_offset * 4 + nMin, ENDIAN)
            bszBackup = b.toInt() and 0x0ff
            Common.set_byte_for_int(
                pbOUT, pbOUT_offset * 4 + nMin,
                (b + nIncreaseValue).toByte(), ENDIAN
            )
        }

        var i = nMin
        while (i > 1) {
            if (bszBackup <= ((Common.get_byte_for_int(pbOUT, pbOUT_offset * 4 + i, ENDIAN).toInt()) and 0x0ff)) {
                return
            } else {
                val b = Common.get_byte_for_int(pbOUT, pbOUT_offset * 4 + i - 1, ENDIAN)
                bszBackup = b.toInt() and 0x0ff
                Common.set_byte_for_int(pbOUT, pbOUT_offset * 4 + i - 1, (b + 1).toByte(), ENDIAN)
            }
            --i
        }

        val b = Common.get_byte_for_int(pbOUT, pbOUT_offset * 4 + 0, ENDIAN)
        bszBackup = b.toInt() and 0x0ff
        Common.set_byte_for_int(pbOUT, pbOUT_offset * 4 + 0, (b + nIncreaseValue).toByte(), ENDIAN)
    }
     ...
     */
    fun chartoint32_for_SEED_CTR(`in`: ByteArray?, inLen: Int): IntArray {
        val data: IntArray
     ...
     */
    fun int32tochar_for_SEED_CTR(`in`: IntArray?, inLen: Int): ByteArray {
        var i: Int
    ...
    private fun KISA_SEED_Encrypt_Block_forCTR(
        `in`: IntArray,
        in_offset: Int,
        out: IntArray,
        out_offset: Int,
    ...
    fun SEED_CTR_init(pInfo: KISA_SEED_INFO?, enc: KISA_ENC_DEC, pszUserKey: ByteArray?, pbszCTR: ByteArray?): Int {
        val ABCD = IntArray(4)
        val T = IntArray(2)

        if (null == pInfo || null == pszUserKey || null == pbszCTR) return 0
    ...
    fun SEED_CTR_Process(pInfo: KISA_SEED_INFO?, `in`: IntArray?, inLen: Int, out: IntArray?, outLen: IntArray): Int {
        var nCurrentCount = 0
        var in_offset = 0
        var out_offset = 0
        val pbszCounter_offset = 0

        if (null == pInfo || null == `in` || null == out || 0 > inLen) return 0

        val pdwCounter = pInfo.ivec
    ...
    fun SEED_CTR_Close(pInfo: KISA_SEED_INFO, out: IntArray?, out_offset: Int, outLen: IntArray): Int {
        val nPaddngLeng = -(pInfo.buffer_length)
    ...
    fun SEED_CTR_Encrypt(
        ...
        val nInputTextPadding = (BLOCK_SIZE_CTR - (message_length % BLOCK_SIZE_CTR)) % BLOCK_SIZE_CTR
        val newmessage = ByteArray(message_length + nInputTextPadding)
        System.arraycopy(message, message_offset, newmessage, 0, message_length)

        val pbszOutputText = ByteArray(message_length)

        SEED_CTR_init(info, KISA_ENC_DEC.KISA_ENCRYPT, pbszUserKey, pbszCTR)

        outlen = ((newmessage.size / BLOCK_SIZE_CTR)) * BLOCK_SIZE_CTR_INT
        ...
        var cdata: ByteArray? = int32tochar_for_SEED_CTR(outbuf, nOutLeng[0] - nPaddingLeng[0])
    ...
    fun main(args: Array<String>) {
        ...
        var PLAINTEXT_LENGTH = 5
        var CIPHERTEXT_LENGTH = 15


        /*******************************************************************************************
         * 방법 1
         */
        print("[ Test SEED CTR reference code ] 방법 1" + "\n")
        print("\n\n")
        print("[ Test Encrypt mode ]" + "\n")
        print("Key\t\t: ")
        ...
        print("\n")
        print("CTR\t\t: ")
        for (i in 0..15) print(Integer.toHexString(0xff and bszCTR[i].toInt()) + " ")
        print("\n")
         ...
         * 방법 1
         */
        var defaultCipherText: ByteArray? = SEED_CTR_Encrypt(pbUserKey, bszCTR, pbData, 0, PLAINTEXT_LENGTH)


        var defaultPlainText = SEED_CTR_Decrypt(pbUserKey, bszCTR, defaultCipherText, 0, PLAINTEXT_LENGTH)



        print("\n\nCiphertext(Enc)\t: ")
        for (i in 0 until PLAINTEXT_LENGTH) print(Integer.toHexString(0xff and defaultCipherText!![i].toInt()) + " ")
        print("\n")


        print("Plaintext(Dec)\t: ")
        for (i in 0 until PLAINTEXT_LENGTH) print(Integer.toHexString(0xff and defaultPlainText[i].toInt()) + " ")
        print("\n")
        ...
        for (i in 0..9) {
            ...
            print("\n\nCiphertext(Enc)\t: ")
            for (j in 0 until PLAINTEXT_LENGTH + test_num) print(Integer.toHexString(0xff and defaultCipherText[j].toInt()) + " ")
            print("\n")


            print("Plaintext(Dec)\t: ")
            for (j in 0 until PLAINTEXT_LENGTH + test_num) print(Integer.toHexString(0xff and defaultPlainText[j].toInt()) + " ")
            print("\n")
        ...
        var info = KISA_SEED_INFO()
        var pdmessage_length = PLAINTEXT_LENGTH

        var process_blockLeng = 32
        var outbuf: IntArray? = IntArray(process_blockLeng)
        ...
        val nPaddingLeng = intArrayOf(0)
        var pbszPlainText: ByteArray? = ByteArray(process_blockLeng)
        var pbszCipherText: ByteArray? = ByteArray(pdmessage_length)

        var j = 0
        while (j < pdmessage_length - process_blockLeng) {
         ...
         */
        info = KISA_SEED_INFO()
        pdmessage_length = pbszCipherText!!.size

        process_blockLeng = 32
        outbuf = IntArray(process_blockLeng)

        SEED_CTR_init(info, KISA_ENC_DEC.KISA_DECRYPT, pbUserKey, bszCTR)

        var cipherText = ByteArray(process_blockLeng)
        pbszPlainText = ByteArray(pdmessage_length)
        ...
        for (i in 0 until PLAINTEXT_LENGTH) print(Integer.toHexString(0xff and pbszPlainText[i].toInt()) + " ")
        print("\n")
        ...
        pbszCipherText = ByteArray(pdmessage_length)
        ...
        cipherText = ByteArray(process_blockLeng)
        pbszPlainText = ByteArray(pdmessage_length)
