...
object KISA_SEED_ECB {
    ...
    /********************************* Encryption  */
    private fun KISA_SEED_Encrypt_Block_forECB(
        `in`: IntArray,
        in_offset: Int,
        out: IntArray,
        out_offset: Int,
    ...
    fun chartoint32_for_SEED_ECB(`in`: ByteArray, inLen: Int): IntArray {
        val data: IntArray
    ...
    fun int32tochar_for_SEED_ECB(`in`: IntArray?, inLen: Int): ByteArray {
        var i: Int
    ...
    fun SEED_ECB_Encrypt(pbszUserKey: ByteArray?, pbData: ByteArray, offset: Int, length: Int): ByteArray {
        ...
        val pbszPlainText = pbData
        val nPlainTextLen = length
        ...
        val pbszCipherText = ByteArray(nPlainTextLen + nPlainTextPadding)
    ...
    fun KISA_SEED_Decrypt_Block_forECB(
        `in`: IntArray,
        in_offset: Int,
        out: IntArray,
        out_offset: Int,
    ...
    fun SEED_ECB_Decrypt(pbszUserKey: ByteArray?, pbData: ByteArray?, offset: Int, length: Int): ByteArray? {
        val info = KISA_SEED_INFO()
        var outbuf: IntArray?
        var data: IntArray?
        var cdata: ByteArray?
        ...
        val pbszCipherText = pbData
        val nCipherTextLen = length
        ...
        val pbszPlainText = ByteArray(nCipherTextLen)
        ...
        if (SEED_ECB_Close(info, outbuf, nRetOutLeng[0], nPaddingLeng) > 0) {
            ...
            var message_length = nRetOutLeng[0] - nPaddingLeng[0]

            if (message_length < 0) {
                message_length = 0
            }
            val result = ByteArray(message_length)
    ...
    fun SEED_ECB_init(pInfo: KISA_SEED_INFO?, enc: KISA_ENC_DEC, pbszUserKey: ByteArray?): Int {
    ...
    fun SEED_ECB_Process(pInfo: KISA_SEED_INFO?, `in`: IntArray?, inLen: Int, out: IntArray?, outLen: IntArray): Int {
    ...
    fun SEED_ECB_Close(pInfo: KISA_SEED_INFO?, out: IntArray?, out_offset: Int, outLen: IntArray): Int {
        val nPaddngLeng: Int
        var i: Int
        outLen[0] = 0
        ...
        if (KISA_ENC_DEC._KISA_ENCRYPT == pInfo!!.encrypt) {
            ...
            while (i < BLOCK_SIZE_SEED) {
                Common.set_byte_for_int(pInfo.ecb_buffer, i, nPaddngLeng.toByte(), ENDIAN)
                i++
            }
            KISA_SEED_Encrypt_Block_forECB(pInfo.ecb_buffer, 0, out, (out_offset) / 4, pInfo.seed_key)
            outLen[0] = BLOCK_SIZE_SEED
        } else {
            nPaddngLeng = Common.get_byte_for_int(pInfo.ecb_last_block, BLOCK_SIZE_SEED - 1, ENDIAN).toInt()
     ...
     */
    fun SeedRoundKey(
        pdwRoundKey: IntArray,  /* [out] round keys for encryption or decryption */
        pbUserKey: ByteArray /* [in] secret user key*/
    ) {
        val ABCD = IntArray(4) // Iuput/output values at each rounds
        val T = IntArray(2) // Temporary variable
        val K = pdwRoundKey // Pointer of round keys
        ...
        T[0] = ABCD[ABCD_A] + ABCD[ABCD_C] - KC15
        T[1] = ABCD[ABCD_B] - ABCD[ABCD_D] + KC15
        K[30] = SS0[GetB0(T[0]).toInt() and 0x0ff] xor SS1[GetB1(
            T[0]
        ).toInt() and 0x0ff] xor SS2[GetB2(T[0]).toInt() and 0x0ff] xor SS3[GetB3(
            T[0]
        ).toInt() and 0x0ff] // K_16,0
        K[31] = SS0[GetB0(T[1]).toInt() and 0x0ff] xor SS1[GetB1(
            T[1]
        ).toInt() and 0x0ff] xor SS2[GetB2(T[1]).toInt() and 0x0ff] xor SS3[GetB3(
            T[1]
        ).toInt() and 0x0ff] // K_16,1
    }
    ...
    fun main(args: Array<String>) {
        ...
        var pbCipher: ByteArray? = ByteArray(50)
        var pbPlain: ByteArray? = ByteArray(16)

        print("\n")
        print("[ Test SEED reference code ECB ]" + "\n")
        print("\n\n")
        ...
        print("\n")
        print("Plaintext\t: ")
        for (i in 0..15) print(Integer.toHexString(0xff and pbData[i].toInt()) + " ")
        print("\n\n\n")
        ...
        print("Ciphertext(SEED_ECB_Encrypt)\t: ")
        for (i in 0..31) print(Integer.toHexString(0xff and pbCipher[i].toInt()) + " ")
        print("\n")

        print("Plaintext(SEED_ECB_Decrypt)\t: ")
        for (i in 0..15) print(Integer.toHexString(0xff and pbPlain!![i].toInt()) + " ")
        print("\n\n\n")
        ...
        for (i in 0..15) print(Integer.toHexString(0xff and pbCipher[i].toInt()) + " ")
        print("\n")

        print("Plaintext(dec)\t: ")
        for (i in 0..1) print(Integer.toHexString(0xff and pbPlain!![i].toInt()) + " ")
        print("\n\n")
        ...
        print("Plaintext(dec)\t: ")
        for (i in 0..17) print(Integer.toHexString(0xff and pbPlain!![i].toInt()) + " ")
        print("\n\n")
         ...
         * 테스트벡터 1
         */
        var PLAINTEXT_LENGTH = 16
        var CIPHERTEXT_LENGTH: Int
        ...
        val nRetOutLeng = intArrayOf(0)
        val nPaddingLeng = intArrayOf(0)
        var pbszPlainText = ByteArray(process_blockLeng)
        ...
        var pbszCipherText: ByteArray? = ByteArray(PLAINTEXT_LENGTH + nPlainTextPadding)

        i = 0
        while (i < PLAINTEXT_LENGTH - process_blockLeng) {
        ...
        while (i < CIPHERTEXT_LENGTH) {
            print(Integer.toHexString(0xff and pbszCipherText!![i].toInt()) + " ")
            i++
        }
        ...
        var EncryptedMessage_length = CIPHERTEXT_LENGTH
        if (EncryptedMessage_length % BLOCK_SIZE_SEED > 0) {
            ...
            val cipherText = ByteArray(process_blockLeng)
            pbszPlainText = ByteArray(EncryptedMessage_length)

            i = 0
            while (i < EncryptedMessage_length - process_blockLeng) {
            ...
            if (SEED_ECB_Close(info, outbuf, nRetOutLeng[0], nPaddingLeng) > 0) {
                ...
                val message_length = i + remainleng - nPaddingLeng[0]
        ...
        while (i < PLAINTEXT_LENGTH) {
            print(Integer.toHexString(0xff and result!![i].toInt()) + " ")
            i++
        }
        print("\n\n")
        ...
        pbszCipherText = ByteArray(PLAINTEXT_LENGTH + nPlainTextPadding)
        ...
        if (EncryptedMessage_length % BLOCK_SIZE_SEED > 0) {
            ...
            if (SEED_ECB_Close(info, outbuf, nRetOutLeng[0], nPaddingLeng) > 0) {
                ...
                result = ByteArray(PLAINTEXT_LENGTH)
    ...
    class KISA_SEED_INFO {
        ...
        var ecb_buffer: IntArray = IntArray(4)
        var buffer_length: Int
        var ecb_last_block: IntArray = IntArray(4)
        ...
        init {
            ...
            seed_key.init()
            ecb_buffer[3] = 0
            ecb_buffer[2] = ecb_buffer[3]
            ecb_buffer[1] = ecb_buffer[2]
            ecb_buffer[0] = ecb_buffer[1]
            buffer_length = 0
            ecb_last_block[3] = 0
            ecb_last_block[2] = ecb_last_block[3]
            ecb_last_block[1] = ecb_last_block[2]
            ecb_last_block[0] = ecb_last_block[1]
            last_block_flag = 0
        }
    }
