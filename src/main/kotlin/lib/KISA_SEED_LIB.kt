...
class KISA_SEED_LIB {
    fun SEED_KeySched(mKey: ByteArray, rKey: IntArray) {
        val ABCD = IntArray(4)
        val T = IntArray(2)
        ...
        rKey[30] = SS0[GetB0(T[0])
            .toInt() and 0x0FF] xor SS1[GetB1(
            T[0]
        ).toInt() and 0x0FF] xor
                SS2[GetB2(T[0])
    ...
    fun SEED_Encrypt(pOut: IntArray, pIn: IntArray, rKey: IntArray) {
        val LR = IntArray(4)
        val T = IntArray(2)
    ...
    fun SEED_Decrypt(pOut: IntArray, pIn: IntArray, rKey: IntArray) {
        val LR = IntArray(4)
        val T = IntArray(2)
    ...
    companion object {
        ...
        private fun RoundKeyUpdate0(T: IntArray, K: IntArray, K_offset: Int, ABCD: IntArray, KC: Int) {
            ...
            ABCD[0] = ((ABCD[0] shr 8) and 0x00ffffff) xor (ABCD[1] shl 24)
            ABCD[1] = ((ABCD[1] shr 8) and 0x00ffffff) xor (T[0] shl 24)
        }
        ...
        private fun RoundKeyUpdate1(T: IntArray, K: IntArray, K_offset: Int, ABCD: IntArray, KC: Int) {
            ...
            ABCD[2] = (ABCD[2] shl 8) xor ((ABCD[3] shr 24) and 0x000000ff)
            ABCD[3] = (ABCD[3] shl 8) xor ((T[0] shr 24) and 0x000000ff)
        }
        ...
        private fun Byte2Word(src: ByteArray, src_offset: Int): Int {
            return ((src[src_offset].toInt() and 0x0FF) shl 24) or ((src[src_offset + 1].toInt() and 0x0FF) shl 16) or ((src[src_offset + 2].toInt() and 0x0FF) shl 8) or ((src[src_offset + 3].toInt() and 0x0FF))
        }
    }
}